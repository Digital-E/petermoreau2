...
export async function getAllPostsForHome(lang, previewData) {
  ...
  return data.allActualites;
}

export async function getAllPostsForHomePaginate(lang, after) {

  const data = await fetchAPI(
    `
        query AllActualites($lang: String!, $after: String!) {
          allActualites(sortBy: date_DESC, lang: $lang, after: $after) {
            pageInfo {
              hasNextPage
              startCursor
              endCursor
